document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Perbaikan: Pastikan data awal dimuat saat halaman pertama kali dibuka
    ...
    const setEndDateLimit = () => {
        ...
        // Reset batasan jika tanggal mulai tidak ada
        if (!startDateValue) {
        ...
        // PERUBAHAN BARU: Logika untuk Cuti Tahunan
        if (leaveType === 'Cuti Tahunan') {
            const maxDate = new Date(startDate);
            // Tambahkan 2 hari untuk mendapatkan total 3 hari (hari ke-1, ke-2, ke-3)
            maxDate.setDate(startDate.getDate() + 2); 
            // Atur batas maksimal tanggal akhir
            endDateInput.setAttribute('max', formatDate(maxDate));
            return;
        }

        // Logika untuk tipe cuti lainnya
        let maxDays;
        switch (leaveType) {
            case 'Cuti Lahiran':
    ...
    leaveForm.addEventListener('submit', async (e) => {
        ...
        if (leaveType === 'Cuti Tahunan') {
            // PERUBAHAN BARU: Validasi jumlah hari cuti
            if (days > 3) {
